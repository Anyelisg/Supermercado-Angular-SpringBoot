...
import com.melit.supermercado.repository.ClienteRepository;
import com.melit.supermercado.service.ClienteService;
...
/**
 * REST controller for managing {@link com.melit.supermercado.domain.Cliente}.
 */
@RestController
@RequestMapping("/api")
public class ClienteResource {
     ...
     */
    @GetMapping("/clientes")
    public ResponseEntity<List<ClienteDTO>> getAllClientes(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
    ...
    /**
     * {@code GET  /clientes/by-documento/{documento} : get all the clientes by documento
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of clientes in body.
     */
    @GetMapping("/clientes/by-documento/{documento}")
    public ResponseEntity<List<ClienteDTO>> getAllByDocumento(@PathVariable String documento, Pageable pageable) {
        log.debug("REST request to get a page of Coches");
        Page<ClienteDTO> page = clienteService.clienteByDocumento(documento, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
