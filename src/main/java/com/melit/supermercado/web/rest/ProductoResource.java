...
/**
 * REST controller for managing {@link com.melit.supermercado.domain.Producto}.
 */
@RestController
@RequestMapping("/api")
public class ProductoResource {
    ...
    /**
     * {@code GET  /productos/by-codigo/{codigo} : get allByCodigo the producto.
     *
    ...
    public ResponseEntity<List<ProductoDTO>> getAllByModelo(@PathVariable String codigo, Pageable pageable) {
        log.debug("REST request to get a page of Coches");
        Page<ProductoDTO> page = productoService.productoByCodigo(codigo, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
